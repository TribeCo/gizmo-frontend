...
const ContactUsTextfields = () => {
    ...
    const deleteAddress = async (addressId) => {
        ...
        const confirmDeletion = window.confirm('Are you sure you want to delete this address?');
        if (!confirmDeletion) {
    ...
    return (
        ...
        >
            ...
            >
                <Grid container spacing={{xs:2, md:4}}> {/* Increased spacing between Grid items */}
                    <Grid item xs={12} md={4}>
                        <Typography sx={{ padding: 1, paddingLeft: 3, fontSize: 16, fontWeight: 500 }}>نام و نام خانوادگی:</Typography>
                        <TextField name="name" onChange={handleChange} variant="outlined" fullWidth sx={{ backgroundColor: '#FFFFFF', '& .MuiOutlinedInput-root': { borderRadius: '20px' }, borderRadius: '20px' }} />
                    </Grid>
                    <Grid item xs={12} md={4}>
                        <Typography sx={{ padding: 1, paddingLeft: 3, fontSize: 16, fontWeight: 500 }}>آدرس ایمیل:</Typography>
                        <TextField name="email" onChange={handleChange} variant="outlined" fullWidth sx={{ backgroundColor: '#FFFFFF', '& .MuiOutlinedInput-root': { borderRadius: '20px' }, borderRadius: '20px' }} />
                    </Grid>
                    <Grid item xs={12} md={4} sx={{display: {xs: 'none', md: 'block'}}}>
                        <Typography sx={{ padding: 1, paddingLeft: 3, fontSize: 16, fontWeight: 500 }}>توضیحات بیشتر:</Typography>
                        <TextField name="text" onChange={handleChange} variant="outlined" fullWidth multiline rows={4} sx={{ backgroundColor: '#FFFFFF', '& .MuiOutlinedInput-root': { borderRadius: '20px', height: '155px' }, borderRadius: '20px' }} />
                    </Grid>
                    <Grid item xs={12} md={4} sx={{ position: 'relative', bottom: {xs: 0, md:'120px'} }}>
                        <Typography sx={{ padding: 1, paddingLeft: 3, fontSize: 16, fontWeight: 500 }}>شماره تلفن:</Typography>
                        <TextField name="phoneNumber" onChange={handleChange} variant="outlined" fullWidth sx={{ backgroundColor: '#FFFFFF', '& .MuiOutlinedInput-root': { borderRadius: '20px' }, borderRadius: '20px' }} />
                    </Grid>
                    <Grid item xs={12} md={4} sx={{ position: 'relative', bottom: {xs: 0, md:'120px'} }}>
                        <Typography sx={{ padding: 1, paddingLeft: 3, fontSize: 16, fontWeight: 500 }}>موضوع پیام شما:</Typography>
                        <TextField name="title" onChange={handleChange} variant="outlined" fullWidth sx={{ backgroundColor: '#FFFFFF', '& .MuiOutlinedInput-root': { borderRadius: '20px' }, borderRadius: '20px' }} />
                    </Grid>
