...
export default function DeliveryPageInfo() {
	const [addresses, setAddresses] = useState([
		{
			...
			straight_address:
				"کوچه فرانسه، خونه اکبر پشت خونه حاجر تبنمی ستبسنمبیتب سنمتبیس نمبتیم نبتسیم نتبنم",
			current: true,
		},
		{
			id: "2",
			...
			postal_code: 6542542578620,
			straight_address:
				"کوچه فرانسه، خونه اکبر پشت خونه حاجر تبنمی ستبسنمبیتب سنمتبیس نمبیم نتبنم",
			current: true,
		},
	]);
	...
	const [selectedAddress, setSelectedAddress] = useState(
		`${addresses[0].province}, ${addresses[0].city}, ${addresses[0].straight_address}, کد پستی: ${addresses[0].postal_code}`,
	);
			...
			<section className="savedAddresses py-4 rounded-lg px-[4%] flex flex-col w-full">
						...
						<Stack
							...
							<FormControl>
								<FormLabel
									...
									style={{ overflow: "hidden" }}></FormLabel>
								<RadioGroup
										...
										if (address.current) {
											return (
												<FormControlLabel
													...
													onClick={() => {
														setSelectedAddress(
															`${address.province}, ${address.city}, ${address.straight_address}, کد پستی: ${address.postal_code}`,
														);
													}}
							...
							<Grid
								...
								<input
									...
									onChange={(e) =>
										setNewAddressData({
											...newAddressData,
											postalCode: parseInt(e.target.value, 10) || 0,
										})
									}
									onInput={(e) =>
				...
				<Stack className="mt-8 px-[4%]">
					<Typography
						...
						fontSize={18}
						fontWeight={900}
						className="font-bold pb-6 text-[#213346]">
						اطلاعات گیرنده:
					</Typography>
					...
					<FormControl className="w-full">
						...
						<RadioGroup
							...
							<div className="flex justify-between px-8 w-full md:flex-col">
								<FormControlLabel
									label={"ارسال با پست پیشتاز: هزینه 35هزار تومان"}
									control={<Radio />}
									value={1}
								/>
								<FormControlLabel
				...
				<div className="w-full rounded-lg bg-[#EEEEEE] my-8"></div>
			</section>
		</div>
	);
}
