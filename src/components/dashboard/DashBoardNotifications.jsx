		...
		<Paper
			...
			<section className=" py-4 px-[4%] w-[60rem] md:w-[24rem] flex flex-col h-full lg:w-full rounded-xl shadow-lg">
				<div className="flex border-b border-[#EDEDED] justify-between py-2 mb-4">
					<h3 className="font-bold text-lg">پیغام ها</h3>
					...
					<div className="flex gap-2 items-center cursor-pointer hover:bg-[#6d4cff1e] transition-all p-2 rounded-lg">
						...
						<span
							...
							className="text-palette-blue font-bold md:text-xs select-none">
				...
				<section className="flex flex-col mt-4 gap-2 overflow-scroll h-full">
					{notifications.map((notif, index) => (
						<div
							key={index + 1}
							className="notificationCard p-2 rounded-lg px-6 flex border-b-2 hover:bg-[#00000012] transition-all border-[#EDEDED] h-40 hover:h-44 justify-between">
							<div className="rightPart flex flex-col justify-between">
								<div className="notifCardHeader flex flex-col gap-5">
									...
									<p className="description text-md md:text-xs">{notif.text}</p>
								</div>

								<a
									href={notif.abs_link}
									className="notifCardLink flex flex-row-reverse items-center justify-end gap-2">
									<Image
										alt="Arrow Left"
										src={ArrowLeft}></Image>

									<span className="text-palette-blue md:text-sm select-none">
										مشاهده جزئیات بیشتر
									</span>
								</a>
							</div>
