import React, { Component } from 'react'
import Swiper from 'swiper/bundle';

import 'swiper/css/bundle';

export default class introSlider extends Component {

  constructor (props) {
    super(props)

    const swiper = new Swiper(".swiper", {

      direction: 'horizontal',
      loop: true,

      pagination: {
        el: '.swiper-pagination',
      },

      navigation: {
        nextEl: '.swiper-button-next',
        prevEl: '.swiper-button-prev',
      }


    })
  }

  render() {

    let {id, imageAdd, title, description} = this.props

    return (
      <div className="slider-container">
      <div className="content-wrapper">

        <div className="slider-images-wrapper">

          <div className="swiper">

            <div className="swiper-pagination"></div>


            <div className="swiper-slide first-slide">
              <img src="" className='swiper-slide-image' alt="" />
            </div>

            <div className="swiper-slide second-slide">
              <img src="" className='swiper-slide-image' alt="" />
            </div>

            <div className="swiper-button-prev"><img src="" alt="" /></div>
            <div className="swiper-button-next"><img src="" alt="" /></div>

          </div>

        </div>

        <div className="slider-content-wrapper">
          <div className="product-info">

            <h3 className="product-info-title">
              {title}
            </h3>

            <p className="product-info-paragraph">
              {description}
            </p>

          </div>

          <button className="product-info-directorBtn">
            <span>!هم اکنون خرید کنید</span>
            <img src="" alt="" />
          </button>
        </div>

      </div>
    </div>
    )
  }
}
